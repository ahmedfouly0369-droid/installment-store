...
export function OverdueInstallments() {
  ...
  const q = useQuery({
    ...
    queryFn: () => call<InstallmentRangeReport>('reports:overdue-installments')
  })

  const rows = q.data?.rows ?? []
  const min = settings.data?.overdue_min_days ?? 1
  const max = settings.data?.bad_debt_days ?? 90
              ...
              {rows.length === 0 ? (
                ...
                rows.map(r => (
                  <tr key={r.id}>
                    ...
                    <td>
                      <span className="badge-amber">{r.days_overdue}</span>
                    </td>
